...
[string]$strVC = Read-Host -prompt 'Enter vCenter server name'				# Virtual Center Server name
[string]$logfile = "c:\temp\getOrphanVMDK.log"
[string]$SMTPServer = Read-Host -prompt 'Enter SMTP server'				# Change to a SMTP server in your environment
[string]$mailfrom = Read-Host -prompt 'Enter SMTP from address'		# Change to email address you want emails to be coming from
[string]$mailto = Read-Host -prompt 'Enter address to send report to'			# Change to email address you would like to receive emails
[string]$mailreplyto = Read-Host -prompt 'Enter reply to address'			# Change to email address you would like to reply emails
...
if ($countOrphaned -gt 0) {
	[string]$body = "Possible Orphaned VMDKs Found: `n"
	...
	$body += [string](" - Before removing, please confirm that there are no backup jobs in progress and that no VMs require these VMDKs.")
